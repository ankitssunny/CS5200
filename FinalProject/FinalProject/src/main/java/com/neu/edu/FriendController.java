...
public class FriendController {
	...
	@RequestMapping(value = "/searchAllAcceptedByTab", method = RequestMethod.GET)
	public String searchAllAcceptedFriendsByTab(Model model, HttpServletRequest request){
		HttpSession session = request.getSession();
		CoupleSignUp fromUser = (CoupleSignUp) session.getAttribute("coupleSU");
		ArrayList<CoupleSignUp> coupleFriends = friendsDAO.mytotalFriends(fromUser);
	//	int counterAllAccep = coupleFriends.size();
		//System.out.println("counter all accepted value" + counterAllAccep);
		model.addAttribute("acceptedFriends", coupleFriends);
		return "friendList";
	}
	...
	public @ResponseBody String pendingRequestByMe(Model model, HttpServletRequest request, HttpServletResponse response){
		HttpSession session = request.getSession();
		CoupleSignUp fromUser = (CoupleSignUp) session.getAttribute("coupleSU");
		System.out.println("Just to check if enterinng in pendingsentbyuser");
		...
		{	
			...
			result = result.append("<li class='clearfix'><div class='thumbnail clearfix'>"+
					"<img src='src='/profileImages/"+coupleSignUp.getCoupleName()+".jpg' alt='ALT NAME' "
					+ "class='pull-left  img-circle' onerror=\"this.src='resources//img/Dashboard/defaultProfile.jpg';"
					+ "\" style='margin-right: 10px; width: 150px; height: 150px;'>"+
					"<div class='caption'>"+
					"<h4><a href='#'>"+coupleSignUp.getCoupleName()+"</a></h4><small><b>Couple Info:"
					+ " </b><span class='coupleName couple-info'> <span class='person1-age'> "
					+coupleSignUp.getCoupleInfo().getPersonList().get(0).getAge()+"</span> years old <span "+
					"class='person1-sex'>"+ coupleSignUp.getCoupleInfo().getPersonList().get(0).getSex()+""
					+ "</span> <span class='&'> & </span> <span class='person2-age'>"
					+coupleSignUp.getCoupleInfo().getPersonList().get(1).getAge()+"</span> years old <span class='person2-sex'>"
					+ coupleSignUp.getCoupleInfo().getPersonList().get(0).getSex()+"</span>"+
					"</span> <br /> <span class='coupleName location'><strong>Current Location:</strong><span class='coupleName location'>"
					+coupleSignUp.getCoupleInfo().getLocation()+"</span><br /> <b>What are we looking for: </b><span class='username story'>"
					+coupleSignUp.getCoupleInfo().getLookingfor()+"</small>	</div></div></li>");

		}
		//	model.addAttribute("pendingReqSent", fzone);
	...
	public @ResponseBody String pendingRequestForMe(Model model, HttpServletRequest request, HttpServletResponse response){
		...
		System.out.println("Just to check if enterinng in pendingreceivedbyuser");
		...
		{	
			...
			result = result.append("<li class='clearfix'><div class='thumbnail clearfix'>"+
					"<img src='src='/profileImages/"+coupleSignUp.getCoupleName()+".jpg' alt='ALT NAME' class='pull-left  img-circle'"
					+ " onerror=\"this.src='resources//img/Dashboard/defaultProfile.jpg';\" style='margin-right: 10px; width: 150px; height: 150px;'>"+
					"<div class='caption'><a  class='btn btn-danger icon  pull-right decline-btn'>Decline</a>"
					+ " <a  class='btn btn-success icon  pull-right accept-btn'>Accept</a>"+
					"<h4><a href='#'>"+coupleSignUp.getCoupleName()+"</a></h4><small><b>Couple Info:"
					+ " </b><span class='coupleName couple-info'> <span class='person1-age'> "
					+coupleSignUp.getCoupleInfo().getPersonList().get(0).getAge()+"</span> years old <span "+
					"class='person1-sex'>"+ coupleSignUp.getCoupleInfo().getPersonList().get(0).getSex()+""
					+ "</span> <span class='&'> & </span> <span class='person2-age'>"+coupleSignUp.getCoupleInfo().getPersonList().get(1).getAge()+
					"</span> years old <span class='person2-sex'>"+ coupleSignUp.getCoupleInfo().getPersonList().get(1).getSex()+"</span"+
					"</span> <br /> <span class='coupleName location'><strong>Current Location:</strong>"
					+ "<span class='coupleName location'>"+coupleSignUp.getCoupleInfo().getLocation()+"</span><br />"
					+ " <b>What are we looking for: </b><span	class='username story'>"
					+coupleSignUp.getCoupleInfo().getLookingfor()+"</small>	</div></div></li>");
		}
		//	model.addAttribute("pendingReqReceived", fzone);
		return result.toString();
	}
	...
	public @ResponseBody String decliningPendingRequest(Model model, HttpServletRequest request, @RequestParam("coupleToDecline") String coupleToDecline){
		HttpSession session = request.getSession();
		CoupleSignUp toUser = (CoupleSignUp) session.getAttribute("coupleSU");
		CoupleSignUp fromUser = coupleInfoDAO.fetchViewCouple(coupleToDecline);
...
//		System.out.println("Just to check if it reached in unfriend");
//		System.out.println("couplename is " + coupleToUnfriend);
		HttpSession session = request.getSession();
		CoupleSignUp toUser = (CoupleSignUp) session.getAttribute("coupleSU");
